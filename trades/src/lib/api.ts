...
export const API_BASE = (import.meta as any).env?.VITE_API_URL || 'https://dhruva-trading.onrender.com'
...
export async function clearDhanCreds(): Promise<DhanStatus> {
  return jsonOrThrow(await fetch(`${API_BASE}/api/settings/dhan`, { method: 'DELETE' }))
}
...
export async function fetchBacktestOOS(req: LiveRequest, signal?: AbortSignal): Promise<OOSResult> {
  const res = await fetch(`${API_BASE}/api/backtest/oos`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(req),
    signal,
...
export async function fetchWalkForward(req: LiveRequest, window: number, step: number, signal?: AbortSignal): Promise<WalkForwardResult> {
  const res = await fetch(`${API_BASE}/api/backtest/walkforward?window=${window}&step=${step}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(req),
    signal,
...
export async function fetchQuote(symbol: string, signal?: AbortSignal): Promise<Quote> {
  return jsonOrThrow(await fetch(`${API_BASE}/api/quote?symbol=${encodeURIComponent(symbol)}`, { signal }))
}
...
export async function fetchBars(symbol: string, interval: Interval, signal?: AbortSignal): Promise<BarsResponse> {
  return jsonOrThrow(
    await fetch(`${API_BASE}/api/bars?symbol=${encodeURIComponent(symbol)}&interval=${interval}`, { signal }),
  )
}