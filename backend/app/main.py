...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        print(f"[WARN] Scheduler cleanup failed: {e}")

app = FastAPI(title="Dhruva backtest engine", version="0.1.0", lifespan=lifespan)

# Allow all origins for production (Render, Vercel, etc.)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
@app.get("/api/health")
def health() -> dict:
    return {"status": "ok", "instruments": len(data.SYMBOL_MAP), "version": "0.1.0"}
...
def ai_review(req: BacktestRequest) -> dict:
    ...
    reviews = heuristic_reviews  # fallback
    source = "heuristic"
    message = ""
